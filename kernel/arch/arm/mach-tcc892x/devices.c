...
static int tcc8920_ochi_fs_init(struct device *dev)
{
	return 0;
}

static struct tccohci_platform_data tcc8920_ohci_fs_platform_data = {
	.port_mode	= USBOHCI_PPM_PERPORT,
	.init		= tcc8920_ochi_fs_init,
};

static struct resource tcc8920_ohci_fs_resources[] = {
	[0] = {
		.start = tcc_p2v(HwUSB11HOST_OHCI_BASE),
		.end   = tcc_p2v(HwUSB11HOST_OHCI_BASE) + 0x0fff,
		.flags = IORESOURCE_MEM,
	},
	[1] = {
		.start = INT_USB11H,
...
struct platform_device ohci_hs_device = {
	.name			= "tcc-ohci",
	.id				= 0,
	.num_resources  = ARRAY_SIZE(tcc8920_ohci_fs_resources),
	.resource       = tcc8920_ohci_fs_resources,
	.dev			= {
		.dma_mask 			= &tcc8920_device_ohci_dmamask,
		.coherent_dma_mask	= 0xffffffff,
		.platform_data		= &tcc8920_ohci_fs_platform_data,
	},
};
#endif
/*----------------------------------------------------------------------
