...
static void tca_gmac_clk_enable(void)
{
	...
	if(gmac_clk) {
		clk_enable(gmac_clk);
	#if defined(CONFIG_TCC_GMAC_RMII_MODE)
		clk_set_rate(gmac_clk, 50*1000*1000);
	#else
...
static void tca_gmac_portinit(void)
{
	...
	defined(CONFIG_TCC_GMAC_GMII_MODE) ||\
	defined(CONFIG_TCC_GMAC_RGMII_MODE)
...
#define GMAC_DRV_STRENGTH_2 2
	...
	#elif defined(CONFIG_TCC_GMAC_RMII_MODE)
	    pHSIOCFG->ETHER_CFG1.bREG.PHY_INFSEL = 4;
	#elif defined(CONFIG_TCC_GMAC_GMII_MODE)
	...
	#if defined(CONFIG_TCC_GMAC_MII_MODE)
		#error "not defined interface"
	#elif defined(CONFIG_TCC_GMAC_RMII_MODE)
	    pHSIOCFG->ETHER_CFG1.bREG.PHY_INFSEL = 4;
	    pHSIOCFG->ETHER_CFG0.bREG.TR = 1;
	    pHSIOCFG->ETHER_CFG0.bREG.RR = 1;
	    pHSIOCFG->ETHER_CFG0.bREG.TXDIV = 1;
...
static void tcc_gmac_set_clk_rate(struct tcc_gmac_priv *priv, unsigned int clk_rate)
{
	...
	switch(clk_rate) {
		case 1250000:
			priv->hw->clk_rate = GMII_CLK_RANGE_150_250M;
	...
	switch(clk_rate) {
		...
		case 25000:
	    	pHSIOCFG->ETHER_CFG1.bREG.CE = 0; //Clock Disable
			pHSIOCFG->ETHER_CFG0.bREG.TXDIV = 19;
	    	pHSIOCFG->ETHER_CFG1.bREG.CE = 1; //Clock Enable
			priv->hw->clk_rate = GMII_CLK_RANGE_150_250M;
			break;
		default:
