...
#define dprintk(msg...)	if (0) { printk( "Vioc_rdma: " msg); }
...
{
	int i;
	unsigned int reg_scale_x, reg_scale_y;
	
	reg_scale_x = (pRDMA->uSCALE.nREG & 0x00000007) >> 0;
	reg_scale_y = (pRDMA->uSCALE.nREG & 0x00070000) >> 16;

	if((scaleX == reg_scale_x) && (scaleY == reg_scale_y))
		return;
