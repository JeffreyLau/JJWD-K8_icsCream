...
int VIOC_CONFIG_PlugIn (unsigned int nType, unsigned int nValue)
{
	...
	while(1) {
		...
		if(loop < 1) {
			printk("VIOC_CONFIG_PlugIn:  path configuration error(2). device is busy. Type:%d Value:%d Status:0x%X\n", nType, nValue, pConfigPath->nREG);
			dump_stack();
			return VIOC_DEVICE_BUSY;
		}
	}
...
int VIOC_CONFIG_PlugOut(unsigned int nType)
{
	...
	while(1) {
		...
		if(loop < 1) {
			printk("VIOC_CONFIG_PlugOut:  path configuration error(2). device is busy. Type:%d Status:0x%X\n", nType, pConfigPath->nREG);
			dump_stack();
			return VIOC_DEVICE_BUSY;
		}
	}
