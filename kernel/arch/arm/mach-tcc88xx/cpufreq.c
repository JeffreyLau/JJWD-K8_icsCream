			      ...
			      unsigned int relation)
{
	...
	if ( android_system_booting_finished == 0 ) {
		target_freq = tcc_voltage_table[NUM_VOLTAGES-1].cpu_freq;
	}
#endif
...
static int tcc_cpufreq_resume(struct cpufreq_policy *policy)
{
	...
	if (do_hibernation ) {
       ...
       printk("1) memory bus  : %4d MHz\n", tca_ckc_getfbusctrl(FBUS_MEM)/10000);
	
		if( do_hibernate_boot) {
			tcc_freq_old_table.cpu_freq = tcc_voltage_table[NUM_VOLTAGES-1].cpu_freq;
			tcc_freq_old_table.ddi_freq = tcc_voltage_table[NUM_VOLTAGES-1].ddi_freq;
			tcc_freq_old_table.mem_freq = tcc_voltage_table[NUM_VOLTAGES-1].mem_freq;
			tcc_freq_old_table.gpu_freq = tcc_voltage_table[NUM_VOLTAGES-1].gpu_freq;
			tcc_freq_old_table.io_freq = tcc_voltage_table[NUM_VOLTAGES-1].io_freq;
			tcc_freq_old_table.vbus_freq = tcc_voltage_table[NUM_VOLTAGES-1].vbus_freq;
			tcc_freq_old_table.vcod_freq = tcc_voltage_table[NUM_VOLTAGES-1].vcod_freq;
			tcc_freq_old_table.smu_freq = tcc_voltage_table[NUM_VOLTAGES-1].smu_freq;
			tcc_freq_old_table.hsio_freq = tcc_voltage_table[NUM_VOLTAGES-1].hsio_freq;
		} else {
			return 0;
		}
	}
	...
	if (do_hibernation ) {
		if( do_hibernate_boot) {
			for(index = 0;index < NUM_VOLTAGES;index++)
				if(gtClockLimitTable[NUM_FREQS - 1].cpu_freq < tcc_voltage_table[index].cpu_freq)
					break;

			index = (index > NUM_FREQS)	? (NUM_FREQS - 1) : (index - 1);

			tcc_freq_old_table.cpu_freq  = tcc_voltage_table[index].cpu_freq;
			tcc_freq_old_table.ddi_freq  = tcc_voltage_table[index].ddi_freq;
			tcc_freq_old_table.mem_freq  = tcc_voltage_table[index].mem_freq;
			tcc_freq_old_table.gpu_freq  = tcc_voltage_table[index].gpu_freq;
			tcc_freq_old_table.io_freq   = tcc_voltage_table[index].io_freq;
			tcc_freq_old_table.vbus_freq = tcc_voltage_table[index].vbus_freq;
			tcc_freq_old_table.vcod_freq = tcc_voltage_table[index].vcod_freq;
			tcc_freq_old_table.smu_freq  = tcc_voltage_table[index].smu_freq;
			tcc_freq_old_table.hsio_freq = tcc_voltage_table[index].hsio_freq;
		} else {
			return 0;
		}
	}
