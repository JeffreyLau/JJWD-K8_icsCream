...
static int _clk_hibernate_enable(struct clk *clk)
{
	int ret = 0;
	if (clk->usecount > 0) {
		if (clk->parent)
			ret = _clk_hibernate_enable(clk->parent);
		if (clk->enable)
			ret = clk->enable(clk);
		else
			pr_warning(
					"clock: %s: unimplemented function 'clk_enable' called\n",
					clk->name);
	} else {
...
static void _clk_hibernate_disable_unused(void)
{
    struct clk *clk;
    int count = 0;
    list_for_each_entry(clk, &clock_list, list) {
        if (clk->flags & CLK_AUTO_OFF && !clk->usecount && clk->disable) {
            clk->disable(clk);
            count++;
            dbg("clock: hibernate clock '%s' disabled\n", clk->name);
        }
    }
    dbg("clock: hibernate disabled %d unused clocks\n", count);
}


int clk_hibernate_init(void)
{
    struct clk **clk;

    for (clk = onchip_clocks; clk < onchip_clocks + ARRAY_SIZE(onchip_clocks); clk++) {
    	unsigned long rate;			// requested rate
	unsigned char temp_name[5];
    	rate = (*clk)->rate;

        if( !do_hibernate_boot )
        	continue;

        if( (*clk)->flags != CLK_AUTO_OFF ) {
    		dbg("1) %s enable %d flag 0x%x rate %x real_rate %x -----------------------------\n", (*clk)->name, (*clk)->usecount, (*clk)->flags, (*clk)->rate, (*clk)->real_rate);
        	continue;
        }

        dbg("+++++++++++++++++++++++++++++ %s enable %d flag 0x%x rate %x real_rate %x \n", (*clk)->name, (*clk)->usecount, (*clk)->flags, (*clk)->rate, (*clk)->real_rate);
        (*clk)->real_rate = 0xFFFFFFFF;
        (*clk)->rate = 0xFFFFFFFF;
        (*clk)->rate = fclk_get_rate((*clk));

        if( (*clk)->usecount > 0 && (long) rate != -1 ) {
    		dbg("start %s enable %d rate %d \n", (*clk)->name, (*clk)->usecount, (*clk)->rate);
        	_clk_hibernate_enable(*clk);
        	clk_forced_set_rate(*clk, rate);
    		dbg("end %s enable %d rate %d \n", (*clk)->name, (*clk)->usecount, (*clk)->rate);
        }

#if 1
	if(strcmp((*clk)->name, "uart0") != 0 && (*clk)->usecount > 0){ 
		// for BT, GPS, Etc using uart, uart clock rate will be set after setting baudrate.
		// so, except console uart, should check only enable of clock for uart
		strncpy(temp_name,(*clk)->name, 4);
		temp_name[4] = NULL;
		if( strcmp(temp_name, "uart") == 0)
        		_clk_hibernate_enable(*clk);
	}
#endif		
    ...
    _clk_hibernate_disable_unused();
    
    pr_info("TCC clock driver hibernate initialized\n");
    return 0;
}

EXPORT_SYMBOL(clk_hibernate_init);
#endif
