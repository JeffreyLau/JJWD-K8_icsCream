...
#if !defined(CONFIG_SND_TCC_DAI2SPDIF)
#if defined(CONFIG_ARCH_TCC892X) || defined(CONFIG_SOC_TCC8925S)	// Planet 20121025 SPDIF exists only on channel01
    {
        .name = "TCC",
        .stream_name = "IEC958",
...
static int tcc_soc_suspend_pre(struct snd_soc_card *card)
{
    if(machine_is_tcc8920()) {
        gpio_set_value(TCC_GPEXT1(1), 1);        // Codec power on
        gpio_set_value(TCC_GPEXT2(0), 0);        // Line ouput mute
    }
}

static int tcc_soc_resume_post(struct snd_soc_card *card)
{
    if(machine_is_tcc8920()) {
        gpio_set_value(TCC_GPEXT1(1), 1);        // Codec power on
        gpio_set_value(TCC_GPEXT2(0), 1);        // Line ouput un-mute
    }
}

#else
#define tcc_soc_suspend_pre NULL
#define tcc_soc_resume_post NULL
#endif
...
static struct snd_soc_card tcc_soc_card = {
	...
	.suspend_pre = tcc_soc_suspend_pre,
	.resume_post = tcc_soc_resume_post,
	.dai_link  = tcc_dai,
