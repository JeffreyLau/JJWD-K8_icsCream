...
static int alsa_get_intr_num(struct snd_pcm_substream *substream)
{
    if (substream) {
        ...
        return INT_ADMA1;
#else
        #error "Need to impplement this area"   
#endif
    }
    return -1;
}
...
{
#if defined(CONFIG_ARCH_TCC892X)
    volatile PADMA1 pADMA = (volatile PADMA1)tcc_p2v(BASE_ADDR_ADMA1);	// Planet 20120531
#else
    #error "Need to impplement this area"   
#endif
...
{
#if defined(CONFIG_ARCH_TCC892X)
    volatile PADMA1 pADMA = (volatile PADMA1)tcc_p2v(BASE_ADDR_ADMA1);
#else
    #error "Need to impplement this area"   
#endif
...
#if defined(CONFIG_ARCH_TCC892X)
    volatile PADMA1    pADMA     = (volatile PADMA1)tcc_p2v(BASE_ADDR_ADMA1);		// Planet 20120531
    volatile PADMADAI1 pADMA_DAI = (volatile PADMADAI1)tcc_p2v(BASE_ADDR_DAI1);	// Planet 20120531
...
static int tcc_pcm_trigger(struct snd_pcm_substream *substream, int cmd)
{
    ...
    int ret = 0;
#if defined(CONFIG_ARCH_TCC892X)
	volatile PADMADAI1 pDAI  = (volatile PADMADAI1)tcc_p2v(BASE_ADDR_DAI1);	// Planet 20120531
	volatile PADMASPDIFTX pADMASPDIFTX = (volatile PADMASPDIFTX)tcc_p2v(BASE_ADDR_SPDIFTX1);
	volatile PADMA1    pADMA = (volatile PADMA1)tcc_p2v(BASE_ADDR_ADMA1);		// Planet 20120531
#else
    #error "Need to impplement this area"   
