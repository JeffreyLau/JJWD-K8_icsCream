...
{
	unsigned int lcd_lcdc_num, output_lcdc_num;
	pmap_t pmap;
	...
	if(output_lcdc_num)
	{
		pDISP_OUTPUT[TCC_OUTPUT_HDMI].pVIOC_DispBase = (VIOC_DISP *)tcc_p2v(HwVIOC_DISP1);
		pDISP_OUTPUT[TCC_OUTPUT_HDMI].pVIOC_RDMA_FB= (VIOC_RDMA *)tcc_p2v(HwVIOC_RDMA04);
		pDISP_OUTPUT[TCC_OUTPUT_HDMI].pVIOC_WMIXBase= (VIOC_WMIX *)tcc_p2v(HwVIOC_WMIX1);			

		pDISP_OUTPUT[TCC_OUTPUT_COMPOSITE].pVIOC_DispBase = (VIOC_DISP *)tcc_p2v(HwVIOC_DISP1);
		pDISP_OUTPUT[TCC_OUTPUT_COMPOSITE].pVIOC_RDMA_FB= (VIOC_RDMA *)tcc_p2v(HwVIOC_RDMA04);
		pDISP_OUTPUT[TCC_OUTPUT_COMPOSITE].pVIOC_WMIXBase= (VIOC_WMIX *)tcc_p2v(HwVIOC_WMIX1);			

		pDISP_OUTPUT[TCC_OUTPUT_COMPONENT].pVIOC_DispBase = (VIOC_DISP *)tcc_p2v(HwVIOC_DISP1);
		pDISP_OUTPUT[TCC_OUTPUT_COMPONENT].pVIOC_RDMA_FB= (VIOC_RDMA *)tcc_p2v(HwVIOC_RDMA04);
		pDISP_OUTPUT[TCC_OUTPUT_COMPONENT].pVIOC_WMIXBase= (VIOC_WMIX *)tcc_p2v(HwVIOC_WMIX1);			
	}
	else
	{
		pDISP_OUTPUT[TCC_OUTPUT_HDMI].pVIOC_DispBase = (VIOC_DISP *)tcc_p2v(HwVIOC_DISP0);
		pDISP_OUTPUT[TCC_OUTPUT_HDMI].pVIOC_RDMA_FB= (VIOC_RDMA *)tcc_p2v(HwVIOC_RDMA00);
		pDISP_OUTPUT[TCC_OUTPUT_HDMI].pVIOC_WMIXBase= (VIOC_WMIX *)tcc_p2v(HwVIOC_WMIX0);	

		pDISP_OUTPUT[TCC_OUTPUT_COMPOSITE].pVIOC_DispBase = (VIOC_DISP *)tcc_p2v(HwVIOC_DISP0);
		pDISP_OUTPUT[TCC_OUTPUT_COMPOSITE].pVIOC_RDMA_FB= (VIOC_RDMA *)tcc_p2v(HwVIOC_RDMA00);
		pDISP_OUTPUT[TCC_OUTPUT_COMPOSITE].pVIOC_WMIXBase= (VIOC_WMIX *)tcc_p2v(HwVIOC_WMIX0);	

		pDISP_OUTPUT[TCC_OUTPUT_COMPONENT].pVIOC_DispBase = (VIOC_DISP *)tcc_p2v(HwVIOC_DISP0);
		pDISP_OUTPUT[TCC_OUTPUT_COMPONENT].pVIOC_RDMA_FB= (VIOC_RDMA *)tcc_p2v(HwVIOC_RDMA00);
		pDISP_OUTPUT[TCC_OUTPUT_COMPONENT].pVIOC_WMIXBase= (VIOC_WMIX *)tcc_p2v(HwVIOC_WMIX0);	
	}

	VIOC_WMIX_SetOverlayPriority(pDISP_OUTPUT[TCC_OUTPUT_HDMI].pVIOC_WMIXBase, 0);
...
void TCC_OUTPUT_LCDC_OnOff(char output_type, char output_lcdc_num, char onoff)
{
	...
	if(onoff)
	{
		...
		while(i < 0xF0000)
		{
			...
			{
				dprintk("%s, lcdc disabled!!, count=%d\n", __func__, i);
				break;
			}
	...
	{		
		...
		{
			//Don't set hdmi limit clock table.
		}
		else 
		#endif
		{
...
char TCC_OUTPUT_FB_Update(unsigned int width, unsigned int height, unsigned int bits_per_pixel, unsigned int addr, unsigned int type)
{
	...
	#if defined(CONFIG_MACH_TCC8920ST) && defined(CONFIG_TCC_OUTPUT_ATTACH)
...
int TCC_OUTPUT_FB_MouseMove(unsigned int width, unsigned int height, tcc_mouse *mouse, unsigned int type)
{
	...
	VIOC_WMIX_SetPosition(pWMIXBase, wmix_layer, lcd_w_pos, lcd_h_pos);
	...
	if(interlace_output)
	{
		...
		VIOC_WMIX_SetPosition(pWMIXBase, wmix_layer,  lcd_w_pos, (lcd_h_pos/2) );
	}
	else
	{
...
void TCC_OUTPUT_FB_Update3D(unsigned int src_wd, unsigned int src_ht, char lcdc_num)
{
	...
	else
	{
		...
		//scaler_open((struct inode *)&scaler_3d_inode, (struct file *)&scaler_3d_filp);
		tccxxx_scaler2_ioctl((struct file *)&scaler_3d_filp, TCC_SCALER_IOCTRL_KERENL, &fbscaler);
		//scaler_release((struct inode *)&scaler_3d_inode, (struct file *)&scaler_3d_filp);
...
void TCC_OUTPUT_FB_AttachOutput(char src_lcdc_num, char dst_output, char resolution, char starter_flag)
{
	...
	PVIOC_WDMA	pWDMA;
	PVIOC_SC 	pSC;
	int dst_lcdc_num, fmt, src_wd, src_ht, dst_wd, dst_ht, count;
	...
	/* init index number for displyting */
