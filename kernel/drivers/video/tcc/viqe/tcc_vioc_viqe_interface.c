...
#if defined(CONFIG_MACH_TCC8920) || defined(CONFIG_SOC_TCC8925S)
...
#if defined(CONFIG_FB_TCC_COMPOSITE) && defined(CONFIG_TCC_COMPOSITE_SIGNAL_QUALITY)
extern int tcc_composite_get_started(void);
#endif
						...
						int crop_top, int crop_bottom, int crop_left, int crop_right, int OddFirst)
{
	...
	{
			#if defined(CONFIG_SOC_TCC8925S)
				gRDMA_reg_30Hz = HwVIOC_RDMA03;
				gVIQE_RDMA_num_30Hz = VIOC_VIQE_RDMA_03;
			#else
...
void TCC_VIQE_DI_Run(unsigned int srcWidth, unsigned int srcHeight,	
						int crop_top, int crop_bottom, int crop_left, int crop_right, int OddFirst, int DI_use)
{
	if(gFrmCnt_30Hz == 0)
...
//		if(gDI_mode != VIOC_VIQE_DEINTL_MODE_BYPASS)
	...
	if(useSCALER)
	{
		...
		gusingScale_60Hz = 1;
		printk("%s : Scaler is PLUG-IN and enable\n", __func__);
	}
