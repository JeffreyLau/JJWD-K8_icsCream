...
//	dprintk(" Overlay -> Q_Buffer :: buffer = 0x%x pos_reset:%d start_en:%d\n", curY_phyAddr, pos_reset, start_en);
	...
	if(pos_reset)
	{
		...
		#else
			VIOC_WMIX_SetPosition(pWMIXBase1, VIDEO_CH_NUM,  overlay_cfg.sx, overlay_cfg.sy);
...
static int __init tcc_overlay_probe(struct platform_device *pdev)
{
	...
	BUG_ON(overlay_lcdc_clk == NULL);
		...
		if(lcd_lcdc_num)
		{
			...
			#ifdef CONFIG_SOC_TCC8925S
				pRDMABase6 = (VIOC_RDMA *)tcc_p2v(HwVIOC_RDMA07);
			#else
		...
		else
		{
			...
			#ifdef CONFIG_SOC_TCC8925S
				pRDMABase6 = (VIOC_RDMA *)tcc_p2v(HwVIOC_RDMA03);
