...
static irqreturn_t tcc_vsync_timer_handler(int irq, void *dev_id)
{
	PTIMER pTIMER_reg = (volatile PTIMER)tcc_p2v(HwTMR_BASE);
		...
		if(system_rev == 0x1005 || system_rev == 0x1006 || system_rev == 0x1007 ||system_rev == 0x1008 || system_rev == 0x2002 || system_rev == 0x2003 || system_rev == 0x2004 || system_rev == 0x2005 || system_rev == 0x2006 || system_rev == 0x2007 || system_rev == 0x2008)
		{
...
int tcc_video_last_frame(int res_changed)
{
	...
	if(Output_SelectMode == TCC_OUTPUT_HDMI ||Output_SelectMode== TCC_OUTPUT_COMPOSITE || Output_SelectMode == TCC_OUTPUT_COMPONENT )
	{
		...
		LastFrame = 1;
		LastFrame_for_ResChanged = res_changed;

		spin_lock_irq(&vsync_lock);
		tcc_vsync_pop_all_buffer(&tccvid_vsync.vsync_buffer);
		spin_unlock_irq(&vsync_lock);
		
		if(Last_ImageInfo.Frame_width == 0 || Last_ImageInfo.Frame_height == 0)
		{
		...
		WmixerInfo.img_width 	= Last_ImageInfo.Frame_width;
		WmixerInfo.img_height	= Last_ImageInfo.Frame_height;
	
		//source info
		WmixerInfo.src_y_addr	= (unsigned int)Last_ImageInfo.addr0;
		WmixerInfo.src_u_addr	= (unsigned int)Last_ImageInfo.addr1;
		WmixerInfo.src_v_addr	= (unsigned int)Last_ImageInfo.addr2;

		WmixerInfo.src_fmt		= Last_ImageInfo.fmt;
		...
		if(Last_ImageInfo.Lcdc_layer == 0 && Last_ImageInfo.enable == 0)
		{
		...
		if(tccvid_vsync.deinterlace_mode > 0)
		{
			...
			w = Last_ImageInfo.Frame_width;
			h = Last_ImageInfo.Frame_height;
		...
		lastframe_Info.src_ImgWidth		= Last_ImageInfo.Frame_width;
		lastframe_Info.src_ImgHeight	= Last_ImageInfo.Frame_height;
		lastframe_Info.src_winLeft		= 0;
		lastframe_Info.src_winTop		= 0;
		lastframe_Info.src_winRight		= Last_ImageInfo.Frame_width;
		lastframe_Info.src_winBottom	= Last_ImageInfo.Frame_height;
		...
		lastframe_Info.dest_ImgWidth	= Last_ImageInfo.Image_width;
		lastframe_Info.dest_ImgHeight	= Last_ImageInfo.Image_height;
		lastframe_Info.dest_winLeft		= 0;
		lastframe_Info.dest_winTop		= 0;
		lastframe_Info.dest_winRight	= Last_ImageInfo.Image_width;
		lastframe_Info.dest_winBottom	= Last_ImageInfo.Image_height;
		...
		Last_ImageInfo.Frame_width  = Last_ImageInfo.Image_width;
		Last_ImageInfo.Frame_height = Last_ImageInfo.Image_height;
		Last_ImageInfo.addr0	 	= (unsigned int)lastframe_Info.dest_Yaddr;
#else
		Last_ImageInfo.addr0	 	= (unsigned int)fb_lastframe_pbuf.base;
#endif
		Last_ImageInfo.fmt	 		= TCC_LCDC_IMG_FMT_YUYV; // W x H * 2 = Max 4Mb

		spin_lock_irq(&LastFrame_lockDisp);
		printk("### TCC_LCDC_HDMI_LASTFRAME Start info(%dx%d), res_changed(%d) \n", Last_ImageInfo.Frame_width, Last_ImageInfo.Frame_height, LastFrame_for_ResChanged);

		TCC_HDMI_LAST_FRAME_UPDATE(EX_OUT_LCDC, (struct tcc_lcdc_image_update *)&Last_ImageInfo, (tccvid_vsync.deinterlace_mode && !tccvid_vsync.output_toMemory && !tccvid_vsync.interlace_bypass_lcdc));		
	...
	if(Output_SelectMode == TCC_OUTPUT_HDMI ||Output_SelectMode== TCC_OUTPUT_COMPOSITE || Output_SelectMode == TCC_OUTPUT_COMPONENT )
	{
		...
		Last_ImageInfo.addr0 = WmixerInfo.dst_y_addr;
		Last_ImageInfo.addr1 = WmixerInfo.dst_u_addr;
		Last_ImageInfo.addr2 = WmixerInfo.dst_v_addr;

		spin_lock_irq(&LastFrame_lockDisp);
		printk("### fake TCC_LCDC_HDMI_LASTFRAME Start info(%dx%d), res_changed(%d) \n", Last_ImageInfo.Frame_width, Last_ImageInfo.Frame_height, LastFrame_for_ResChanged);

		switch(Output_SelectMode)
		{
			case TCC_OUTPUT_HDMI:
				TCC_HDMI_DISPLAY_UPDATE(EX_OUT_LCDC, &Last_ImageInfo);
				break;
			case TCC_OUTPUT_COMPONENT:
			#if defined(CONFIG_FB_TCC_COMPONENT)
					tcc_component_update(&Last_ImageInfo);
			#endif
				break;
			case TCC_OUTPUT_COMPOSITE:
			#if defined(CONFIG_FB_TCC_COMPOSITE)
					tcc_composite_update(&Last_ImageInfo);
			#endif
				break;
			default:
				break;
...
void tccfb_output_starter(char output_type, char lcdc_num, struct lcdc_timimg_parms_t *lcdc_timing)
{
	switch(output_type)
	{
		case TCC_OUTPUT_HDMI:
			...
			HDMI_video_width = lcdc_timing->lpc+1;
			HDMI_video_height = lcdc_timing->flc+1;
			break;
...
static int tccfb_set_par(struct fb_info *info)
{
	...
	#if !defined(CONFIG_TCC_OUTPUT_STARTER) && !defined(CONFIG_MACH_TCC8920ST)
		...
		case TCC_LCDC_HDMI_DISPLAY:
			{
				...
				if(LastFrame){
					LastFrame = 0;
					//printk("LastFrame = 0 in TCC_LCDC_HDMI_DISPLAY \n");
				}
				spin_unlock_irq(&LastFrame_lockDisp);
	 	#endif
		...
		case TCC_LCDC_SET_COLORENHANCE:
			{
				...
				if(lcdc_colorenhance.lcdc_num)
					pDISPBase = (VIOC_DISP *)tcc_p2v(HwVIOC_DISP1);
				else
					pDISPBase = (VIOC_DISP *)tcc_p2v(HwVIOC_DISP0);

				VIOC_DISP_SetColorEnhancement(pDISPBase, lcdc_colorenhance.contrast, lcdc_colorenhance.brightness, lcdc_colorenhance.hue);
			}
			break;
	...
	case TCC_LCDC_VIDEO_PUSH_VSYNC:
		...
		if(Output_SelectMode == TCC_OUTPUT_NONE || Output_SelectMode == TCC_OUTPUT_HDMI || Output_SelectMode == TCC_OUTPUT_COMPONENT || Output_SelectMode == TCC_OUTPUT_COMPOSITE)
		{
			...
			if(LastFrame)
			{
				...
				LastFrame = LastFrame_for_ResChanged = 0;
			}
			spin_unlock_irq(&LastFrame_lockDisp);
	#endif
			...
			if(!input_image.output_path && input_image.buffer_unique_id<3)
			{
			...
			if(tccvid_vsync.deinterlace_mode < 0)
			{
				...
				if((tccvid_vsync.outputMode == TCC_OUTPUT_COMPOSITE) && (tccvid_vsync.deinterlace_mode == 1) && (tccvid_vsync.interlace_output == 1) && (input_image.Frame_height == input_image.Image_height))
				{
					if(tccvid_vsync.interlace_bypass_lcdc == 0)
					{
						printk("### interlace_bypass_lcdc set for composite signal!!\n");
						tccvid_vsync.interlace_bypass_lcdc = 1;
					}
				}
			...
			printk("### output_path buffer_unique_id %d \n",input_image.buffer_unique_id);
				...
				if(tccvid_vsync.deinterlace_mode && !tccvid_vsync.output_toMemory && !tccvid_vsync.interlace_bypass_lcdc)		//tccvid_vsync.deinterlace_mode && //
				{
					...
					#else
					int current_time = tcc_vsync_get_time();
					spin_lock_irq(&vsync_lock) ;
