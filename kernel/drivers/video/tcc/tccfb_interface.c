...
int tca_fb_resume(struct platform_device *dev)
{
	...
	if( do_hibernation)
	{
		...
		if(do_snapshot_boot) 
		{
				...
				pLCDC_FB_CH->LIBA0  =fbi->map_dma + (fbinfo->var.xres * fbinfo->var.yoffset * (fbinfo->var.bits_per_pixel/8));
				BITCSET(pLCDC_FB_CH->LIC, 0xFF00000F, 0x0000000C);	// RGB888
				pLCDC_FB_CH->LIO = fbinfo->var.xres * 4;	// offset
				pLCDC_FB_CH->LIS = (fbinfo->var.yres<< 16) |(fbinfo->var.xres); 	//Size
				pLCDC_FB_CH->LIP =  0; 	// position
				pLCDC_FB_CH->LIC |= ( HwLCT_RU | HwLIC_IEN );
				...
				if(ret >= 0)	{
					pLCDC_FB_CH->LIBA0  =fbi->map_dma + (fbinfo->var.xres * fbinfo->var.yoffset * (fbinfo->var.bits_per_pixel/8));
					printk("base: 0x%x xres:%d, yoffset:%d bits_per_pixel:%d \n", pLCDC_FB_CH->LIBA0, fbinfo->var.xres , fbinfo->var.yoffset , fbinfo->var.bits_per_pixel);
					BITCSET(pLCDC_FB_CH->LIC, 0xFF00000F, 0x0000000A);	// rgb565
					pLCDC_FB_CH->LIO = fbinfo->var.xres * 2;	// offset
					pLCDC_FB_CH->LIS = (fbinfo->var.yres<< 16) |(fbinfo->var.xres); 	//Size
					pLCDC_FB_CH->LIP =  0; 	// position
					pLCDC_FB_CH->LIC |= ( HwLCT_RU | HwLIC_IEN );		
					load_image_free();
...
int fb_external_output_update(void)
{
	struct fb_info *info;
	struct tccfb_info *fbi;
	unsigned int base_addr = 0;
	printk(KERN_EMERG "%s:  framebuffer\n",__func__);
	...
	fbi = (struct tccfb_info *)info->par;
	
 	base_addr = fbi->map_dma + fbi->fb->var.xres * fbi->fb->var.yoffset * (fbi->fb->var.bits_per_pixel/8);
	if(fbi->fb->var.yoffset > fbi->fb->var.yres)	{
		base_addr = PAGE_ALIGN(base_addr);
	}
		printk(KERN_EMERG "%d %d %d 0x%x %d :  framebuffer\n" ,fbi->fb->var.xres , fbi->fb->var.yoffset, fbi->fb->var.bits_per_pixel, base_addr, 3);
				tcc_LCDC_set_bgcolor(1, 7,7,7);

	TCC_OUTPUT_FB_Update(fbi->fb->var.xres , fbi->fb->var.yres, fbi->fb->var.bits_per_pixel, base_addr, 3);
				tcc_LCDC_set_bgcolor(1, 9,9,9);

	return 0;
}

EXPORT_SYMBOL(fb_external_output_update);
...
int fb_hibernation_logo(void)
{
	...
	if(do_hibernation ){
		...
		pLCDC_FB_CH->LIBA0 = lcdc_logo.base;

		BITCSET(pLCDC_FB_CH->LIC, 0xFF00000F, 0x0000000C);	// rgb888
		pLCDC_FB_CH->LIO = img_width * ( LCDC_FB_FORMAT / 8);	// offset
		pLCDC_FB_CH->LIS = (img_height<< 16)   | (img_width); 	//Size
		pLCDC_FB_CH->LIP =  0; 	// position
		...
		{
			TCC_OUTPUT_FB_Update(LCDC_FB_WIDTH, LCDC_FB_HEIGHT, LCDC_FB_FORMAT, pLCDC_FB_CH->LIBA0, Output_SelectMode);
