...
/*-------------------------------------------------------------------------*/
static void tcc_USB11HPHY_cfg(void)
{
	unsigned int temp;
	PHSIOBUSCFG pHSIOBUSCFG;
	
	pHSIOBUSCFG = (PHSIOBUSCFG)tcc_p2v(HwHSIOBUSCFG_BASE);

	pHSIOBUSCFG->USB11H_PCFG.bREG.DNPD0 = 1;
	pHSIOBUSCFG->USB11H_PCFG.bREG.DPPD0 = 1;
	msleep(20);
...
static int tcc_ohci_select_pmm(unsigned long reg_base, int mode, int num_port, int *port_mode)
{
	...
	volatile struct PUSB_OHCI ohci_reg;
	ohci_reg = (volatile struct PUSB_OHCI )reg_base;
...
void ohci_port_power_control(int id)
{
	...
	#if defined(CONFIG_SOC_TCC8925S)
	{
		PUSB_OHCI ohci = (PUSB_OHCI)tcc_p2v(HwUSB11HOST_OHCI_BASE);

		/* Port power switch mode & each port is powered individually
		 */
		BITCSET(ohci->HcRhDescriptorA.nREG, Hw9 | Hw8, Hw8);	// NPS 0, PSM 1

		/* port2 power state is only affected by per-port power control (Set/ClearPortPower)
		 * port1 is controlled by the global power switch (Set/ClearGlobalPower)
		 */
		BITSET(ohci->HcRhDescriptorB.nREG, Hw18);	// PPCM bit2 1

		/* Clear the PortPowerStatus (port2 power off)
		 */
		ohci->HcRhPortStatus2.nREG = Hw9;	// port2 LSDA 1
	}
	#endif
}
