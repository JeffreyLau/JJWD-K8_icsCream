...
//	temp = temp | Hw13;				// [13] UTM_COMMONONN   !!Must not setting when Quick Boot
...
static struct clk *usb_hub_clk = NULL;
static int __init ehci_tcc_drv_probe(struct platform_device *pdev)
{
	...
	#if defined(CONFIG_STB_BOARD_UPC)
	{
		...
		if (usb_hub_clk == NULL) {
			usb_hub_clk = clk_get(0, "out1");
			if (IS_ERR(usb_hub_clk)) {
				printk(KERN_WARNING "HDMI: failed to get out1 clock\n");
				usb_hub_clk = NULL;
				return -ENODEV;
			}
		}
		...
		if (usb_hub_clk)
			clk_enable(usb_hub_clk);

		clk_set_rate(usb_hub_clk, 1*1000*1000);

		pCKC->PCLKCTRL19.nREG = 0x24000000;
