...
int tcc_ohci_vbus_ctrl(int id, int on)
	...
	if(system_rev == 0x2009){
		...
		gpio_hs_host_en = TCC_GPEXT5(3);
		gpio_otg1_vbus_en = TCC_GPEXT2(14); //회로도 보고	
...
int tcc_ehci_vbus_ctrl(int on)
{
	...
	else if(machine_is_tcc8920st())
	{
		...
		#if defined(CONFIG_STB_BOARD_HDB892F) || defined(CONFIG_STB_BOARD_UPC)
