...
#if !defined(CONFIG_ARCH_TCC88XX)
extern  int tcc_video_get_displayed(void);
extern  void tcc_video_clear_frame(int idx);
extern  void tcc_video_set_framerate(int fps);
extern  int tcc_video_last_frame(int res_changed);
extern  void tcc_video_skip_frame_start();
extern  void tcc_video_skip_frame_end();
extern  int tcc_video_get_valid_count();
#endif
	...
	switch(cmd)
	{
		...
		case TCC_LCDC_HDMI_LASTFRAME:
		{
			int res_changed;
				...
				if(copy_from_user(&res_changed,(int*)arg, sizeof(res_changed)))
					ret = -EFAULT;
				else
					ret = tcc_video_last_frame(res_changed);
		}
		break;
#endif
