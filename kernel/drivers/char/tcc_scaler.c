...
char M2M_Scaler_Ctrl_Detail(SCALER_TYPE *scale_img)
{
	...
	dprintk("Src:  addr:0x%x, 0x%x, 0x%x,  fmt:%d. \n", scale_img->src_Yaddr, scale_img->src_Uaddr, scale_img->src_Vaddr, scale_img->src_fmt);
	dprintk("Dst:  addr:0x%x, 0x%x, 0x%x,  fmt:%d. \n", scale_img->dest_Yaddr, scale_img->dest_Uaddr, scale_img->dest_Vaddr, scale_img->dest_fmt);
	...
	if((scale_img->src_fmt > SC_IMG_FMT_ARGB6666) && (pSrcBase1 == NULL || pSrcBase2 == NULL)) { // address limitation!!
	...
	if((scale_img->dest_fmt > SC_IMG_FMT_ARGB6666) && (pDstBase1 == NULL || pDstBase2 == NULL)) { // address limitation!!
	...
	// set to WMIX30  
	#if defined(CONFIG_SOC_TCC8925S)
	VIOC_CONFIG_WMIXPath(WMIX50, 0 /* bypass path */);
	#else
...
char Alphablending_Ctrl_Func(SCALER_ALPHABLENDING_TYPE* apb_info)
{
	...
	if(sc_data.is_plugin) {
		VIOC_API_SCALER_SetPlugOut(VIOC_SC1);
		sc_data.is_plugin = 0;		
	}

	VIOC_SC_SetSWReset(100, 12/*RDMA12*/, 100);
	...
	pScalerInfo.BYPASS 			= TRUE /* 1 */;
	pScalerInfo.DST_WIDTH 		= apb_info->dst_width;
	pScalerInfo.DST_HEIGHT 		= apb_info->dst_height;
	pScalerInfo.OUTPUT_POS_X 		= 0;
	pScalerInfo.OUTPUT_POS_Y 		= 0;
	pScalerInfo.OUTPUT_WIDTH 		= pScalerInfo.DST_WIDTH;
	pScalerInfo.OUTPUT_HEIGHT 	= pScalerInfo.DST_HEIGHT;
...
{
	int ret = 0;
	unsigned char sc_num, rdma_num, wdma_num;
	...
	if(sc_data.is_plugin) {
		sc_data.is_plugin = 0;
		#if defined(CONFIG_ARCH_TCC892X)
		VIOC_SC_SetSWReset(VIOC_SC1, 0xff/*RDMA12*/, 0xff/*WDMA03*/);
	...
	if(sc_data.dev_opened == 0)
	{
		...
		if(sc_data.irq_reged) {
			...
			#endif
			VIOC_CONFIG_PlugOut(VIOC_SC1);
			VIOC_SC_SetSWReset(sc_num, rdma_num, wdma_num);
		#else // CONFIG_ARCH_TCC892X
		if(sc_data.irq_reged)	{
...
{	
	...
	if(!sc_data.irq_reged) {
		...
		// set to VRDMA
				...
				VIOC_SC_SetSWReset(VIOC_SC1, 16/*RDMA16*/, 6/*WDMA06*/);
				VIOC_WDMA_SetIreqMask(pWDMABase, VIOC_WDMA_IREQ_EOFR_MASK, 0x0);
				ret = request_irq(INT_VIOC_WD6, tccxxx_scaler_handler, IRQF_SHARED, "scaler", &sc_data);
			#else
