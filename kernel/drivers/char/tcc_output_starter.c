...
#endif /* CONFIG_HDMI_CLK_USE_INTERNAL_PLL */
...
int tcc_hdmi_set_lcdc_config(char lcdc_num, enum VideoFormat hdmi_video_format)
{
    ...
    memcpy((void*)&device,(const void*)&(LCDCTimimgParams[hdmi_video_format]),sizeof(device));
		...
		#if 0//defined(TCC_OUTPUT_STARTER_DIRECT_DISPLAY)
		{
			VIOC_DISP *pDISP;
			stLCDCTR pCtrlParam;
			...
			HDMI_TIMEp.lpw = device.lpw;
			HDMI_TIMEp.lpc = device.lpc + 1;
			HDMI_TIMEp.lswc = device.lswc+ 1;
			HDMI_TIMEp.lewc = device.lewc+ 1;
			HDMI_TIMEp.vdb = device.vdb;
			HDMI_TIMEp.vdf = device.vdf;
			HDMI_TIMEp.fpw = device.fpw;
			HDMI_TIMEp.flc = device.flc;
			HDMI_TIMEp.fswc = device.fswc;
			HDMI_TIMEp.fewc = device.fewc;
			HDMI_TIMEp.fpw2 = device.fpw2;
			HDMI_TIMEp.flc2 = device.flc2;
			HDMI_TIMEp.fswc2 = device.fswc2;
			HDMI_TIMEp.fewc2 = device.fewc2;
			VIOC_DISP_SetTimingParam(pDISP, &HDMI_TIMEp);

			memset(&pCtrlParam, NULL, sizeof(pCtrlParam));
			pCtrlParam.id = device.id;
			pCtrlParam.iv = device.iv;
			pCtrlParam.ih = device.ih;
			pCtrlParam.ip = device.ip;
			pCtrlParam.clen = 0;
			pCtrlParam.r2y = 0;
			pCtrlParam.pxdw = TCC_LCDC_IMG_FMT_RGB888;
			pCtrlParam.dp = device.dp;
			pCtrlParam.ni = device.ni;
			if(device.ni == 0)
				pCtrlParam.advi = 1;
			pCtrlParam.tv = device.tv;
			pCtrlParam.opt = 0;
			pCtrlParam.stn = 0;
			pCtrlParam.evsel = 0;
			pCtrlParam.ovp = 0;
			...
			VIOC_DISP_SetSize (pDISP, device.lpc + 1, device.flc + 1);
...
static int tcc_hdmi_detect_cable(void)
{
	int ret = true;
		...
		#if defined(CONFIG_TCC_OUTPUT_AUTO_DETECTION) || defined(TCC_OUTPUT_STARTER_ATTACH)
...
static int tcc_output_starter_load_image(char *filename)
{
	struct fb_info *fb_ouput;
	struct lcd_panel *lcd_panel = tccfb_get_panel();
	int fd, size, count, copy_count;
	unsigned short *file_data, *src_buf, *dst_buf0, *dst_buf1, *fb_buf;
	...
	{
		printk("%s, Can't open %s\n", __func__, filename);
		return -ENOENT;
	}

	if((count = (unsigned)sys_lseek(fd, (off_t)0, 2)) == 0)
	{
		printk("%s, Can't seek file\n", __func__);
		sys_close(fd);
		return -EIO;
	}
	...
	file_data = (unsigned short *)kmalloc(count, GFP_KERNEL);
	if(!file_data)
	{
		printk("%s: Can't alloc data\n", __func__);
		sys_close(fd);
		return -ENOMEM;
	}

	if((unsigned)sys_read(fd, (char *)file_data, count) != count)
	{
		sys_close(fd);
		return -EIO;
	}

	size = lcd_panel->xres * lcd_panel->yres;

	src_buf = file_data;

	if(pmap_fb.base)
	{
		fb_buf = dst_buf0 = dst_buf1 = (unsigned)ioremap_nocache(pmap_fb.base, pmap_fb.size);

		if(fb_buf)
		{
			while(count > 3)
			{
				unsigned n = src_buf[0];
...
#if defined(CONFIG_ARCH_TCC892X)
void tcc_output_starter_memclr(unsigned char lcdc_num, unsigned char output_type)
{
	...
	if(pmap_fb.base)
	{
	#if defined(TCC_OUTPUT_STARTER_DIRECT_DISPLAY)
		if(tcc_output_starter_load_image("/initlogo.rle") < 0)
			printk("%s load_565rle_image init failed!!\n", __func__);
		else
		...
		if(pBaseAddr)
		{
			...
			if(output_type == STARTER_OUTPUT_COMPOSITE)
			{
				for(count=0; count<pmap_fb.size; count++)
				{
					if(count & 0x01)
						*(pBaseAddr + count) = 0x80;
					else
						*(pBaseAddr + count) = 0x00;
				}
			}
			else
		#endif
...
void tcc_output_starter_skip_logo(unsigned char lcdc_num, unsigned char output_type)
{
	struct clk *clock;
	
	printk("%s LCDC NUM:%d output_type=%d\n", __func__, lcdc_num, output_type);
	
	switch(output_type)
	{
		case STARTER_OUTPUT_HDMI:
			{
				tccfb_output_set_mode(TCC_OUTPUT_HDMI);
			}
			break;
		...
		case STARTER_OUTPUT_COMPOSITE:
			{
				...
				tccfb_output_set_mode(TCC_OUTPUT_COMPOSITE);
			}
			break;

		case STARTER_OUTPUT_COMPONENT:
			{
				tccfb_output_set_mode(TCC_OUTPUT_COMPONENT);
			}
			break;
	}
}
...
void tcc_output_starter_hdmi(unsigned char lcdc_num, unsigned char hdmi_resolution)
{
	...
	#if defined(CONFIG_HDMI_CLK_USE_INTERNAL_PLL)
	{
		...
		pCKC->PCLKCTRL17.nREG = 0x20000015;
	}
	#endif /* CONFIG_HDMI_CLK_USE_INTERNAL_PLL */
	...
	tcc_output_starter_memclr(lcdc_num, STARTER_OUTPUT_HDMI);
...
void tcc_output_starter_composite(unsigned char lcdc_num, unsigned char type)
{
	...
	#if defined(CONFIG_TCC_COMPOSITE_SIGNAL_QUALITY)
		image_fmt = TCC_LCDC_IMG_FMT_YUYV;
	#else
		image_fmt = TCC_LCDC_IMG_FMT_RGB565;
	#endif
	
	tcc_output_starter_memclr(lcdc_num, STARTER_OUTPUT_COMPOSITE);
...
void tcc_output_starter_component(unsigned char lcdc_num, unsigned char type)
{
	...
	tcc_output_starter_memclr(lcdc_num, STARTER_OUTPUT_COMPONENT);
...
int __init tcc_output_starter_init(void)
{
	...
	#if defined(CONFIG_MACH_TCC8920ST) && defined(CONFIG_OUTPUT_SKIP_KERNEL_LOGO)
	...
	pmap_get_info("output_attach", &pmap_attach);
	...
	#else
		#if defined(CONFIG_SOC_TCC8925S) && defined(CONFIG_STB_BOARD_DONGLE)
		lcdc_1st = STARTER_LCDC_0; /* LCDC1: HDMI or Component */
		lcdc_2nd = STARTER_LCDC_1; /* LCDC0: Composite */
		#else
		lcdc_1st = STARTER_LCDC_1; /* LCDC1: HDMI or Component */
		lcdc_2nd = STARTER_LCDC_0; /* LCDC0: Composite */
		#endif /* CONFIG_SOC_TCC8925S */
	#endif
			...
			if(tcc_display_data.output == STARTER_OUTPUT_COMPONENT)
			{
				...
				TCC_OUTPUT_FB_AttachUpdateFlag(lcdc_1st);
			#else
				TCC_OUTPUT_FB_AttachOutput(lcdc_2nd, TCC_OUTPUT_COMPONENT, STARTER_COMPONENT_720P+tcc_display_data.component_resolution, 0);
			#endif
			}
			else
			{
				...
				TCC_OUTPUT_FB_AttachUpdateFlag(lcdc_1st);
			#else
				TCC_OUTPUT_FB_AttachOutput(lcdc_1st, TCC_OUTPUT_COMPOSITE, tcc_display_data.composite_resolution, 0);
			#endif
			}
		#else
			/* 1st Output Setting */
			...
			{
			#if defined(TCC_OUTPUT_AUTO_HDMI_CVBS)
			...
			else
			{
				...
				#if 0//defined(CONFIG_OUTPUT_SKIP_KERNEL_LOGO)
					if(tcc_hdmi_detect_cable() == true)
					{
						/* 1st Output Setting */
						tcc_output_starter_skip_logo(lcdc_1st, STARTER_OUTPUT_HDMI);
						...
						tcc_output_starter_skip_logo(lcdc_1st, STARTER_OUTPUT_COMPOSITE);
				#else
					/* 1st Output Setting */
					if(tcc_display_data.output >= STARTER_OUTPUT_MAX)
						tcc_output_starter_hdmi(lcdc_1st, default_hdmi_resolution);
		...
		switch(tcc_display_data.output)
		{
			...
			case STARTER_OUTPUT_HDMI:
				tcc_output_starter_hdmi(lcdc_1st, 8);//tcc_display_data.hdmi_resolution);
				break;
			case STARTER_OUTPUT_COMPOSITE:
				tcc_output_starter_composite(lcdc_1st, tcc_display_data.composite_resolution);
				break;
