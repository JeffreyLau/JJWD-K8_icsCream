	...
	while (!priv->hw->desc->get_rx_owner(p)) {
		...
		if (unlikely(status == discard_frame))
			...
			if (frame_len > 1400){
				...
				total_byte += frame_len;
				if(time_cnt != 0 && time_cnt % (MAX_GAP_CNT-1) == 0){
					#if 0
					printk("GMAC: %d bytes %d ms: %3d, %3d, %3d, %3d, %3d, %3d, %3d, %3d, %3d, %3d, %3d, %3d, %3d, %3d, %3d, %3d, %3d, %3d, %3d, %3d, %3d, %3d, %3d, %3d, %3d, %3d, %3d, %3d, %3d, %3d \n",
						total_byte,total_time, gap_time[0], gap_time[1], gap_time[2], gap_time[3], gap_time[4], gap_time[5], gap_time[6], gap_time[7], gap_time[8], gap_time[9], 
						gap_time[10], gap_time[11], gap_time[12], gap_time[13], gap_time[14], gap_time[15], gap_time[16], gap_time[17], gap_time[18], gap_time[19], 
						gap_time[20], gap_time[21], gap_time[22], gap_time[23], gap_time[24], gap_time[25], gap_time[26], gap_time[27], gap_time[28], gap_time[29]);
					#else
					//printk("GMAC: %d bytes %d ms:  \n",total_byte,total_time);
					//printk("%d bytes %d ms:  \n",total_byte,total_time);
					#endif
					...
					printk("GMAC: %d bytes avg:%d ms: %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d \n",
						total_byte,total_sum_time, total_times[0], total_times[1], total_times[2], total_times[3], total_times[4], total_times[5], total_times[6], total_times[7], total_times[8], total_times[9], 
					...
					phMscd_Filp = filp_open("/gmaclog", O_RDWR , 0660);
					if (phMscd_Filp == NULL)
					{
					...
					/*
					if (phMscd_Filp != NULL && phMscd_Filp->f_op && (phMscd_Filp->f_op->write) != NULL) 
					phMscd_Filp->f_op->write(phMscd_Filp, gap_time, 30, &phMscd_Filp->f_pos);
					else
						printk("filp_open write!!.\n");
					*/
					if (phMscd_Filp != NULL)
					vfs_write(phMscd_Filp,gap_time,(MAX_CNT-1),&phMscd_Filp->f_pos);
