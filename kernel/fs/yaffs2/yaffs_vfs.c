...
extern unsigned int do_yaffs_remount;
...
#ifdef CONFIG_HIBERNATION
unsigned int yaffs_bg_enable = 0;
#else
...
struct list_head	yaffs_backup_s_inodes;

static void yaffs_inode_backup(struct super_block *sb) {
	struct inode *inode;
	YCHAR buffer[YAFFS_MAX_NAME_LENGTH + 1];

	yaffs_backup_s_inodes = sb->s_inodes;

	list_for_each_entry(inode, &sb->s_inodes, i_sb_list)
	{
		struct yaffs_obj *new_obj = kmalloc(sizeof(struct yaffs_obj), GFP_NOFS);
		struct yaffs_obj *obj = (struct yaffs_obj *) (yaffs_inode_to_obj(inode));
		...
		memcpy(new_obj, obj, sizeof(struct yaffs_obj));
		//yaffs_fill_inode_from_obj(inode, new_obj);

		obj->my_inode = NULL;

		yaffs_inode_to_obj_lv(inode) = new_obj;

		yaffs_get_obj_name(obj, buffer, YAFFS_MAX_NAME_LENGTH + 1);
		//printk("inode backup o[%d]n[%s] %s %d\n", obj->obj_id, buffer, obj->variant_type == YAFFS_OBJECT_TYPE_DIRECTORY ?"dir" : "file", obj->variant_type );
	}

	INIT_LIST_HEAD(&sb->s_inodes);
}

static void yaffs_inode_restore(struct super_block *sb, struct yaffs_dev *dev) {
	struct inode *inode;
	YCHAR buffer[YAFFS_MAX_NAME_LENGTH + 1];

	sb->s_inodes = yaffs_backup_s_inodes;

	list_for_each_entry(inode, &sb->s_inodes, i_sb_list)
	{
		struct yaffs_obj *obj = yaffs_inode_to_obj(inode);
		struct yaffs_obj *new_obj = yaffs_find_by_number(dev, obj->obj_id);

		if( new_obj == NULL ) {
			yaffs_get_obj_name(obj, buffer, YAFFS_MAX_NAME_LENGTH + 1);
			printk("cannot find obj_id o[%d]n[%s] %s %d\n", obj->obj_id, buffer, obj->variant_type == YAFFS_OBJECT_TYPE_DIRECTORY ?"dir" : "file", obj->variant_type );
			obj->my_inode = NULL;
			yaffs_inode_to_obj_lv(inode) = NULL;
		} else {
			yaffs_inode_to_obj_lv(inode) = new_obj;

			inode->i_size = yaffs_get_obj_length(new_obj);
			inode->i_blocks = (inode->i_size + 511) >> 9;
			inode->i_nlink = yaffs_get_obj_link_count(new_obj);

			new_obj->my_inode = inode;
			//yaffs_fill_inode_from_obj(inode, new_obj);
		}

		//yaffs_get_obj_name(obj, buffer, YAFFS_MAX_NAME_LENGTH + 1);
		//printk("inode restore o[%d]n[%s] %s %d\n", obj->obj_id, buffer, obj->variant_type == YAFFS_OBJECT_TYPE_DIRECTORY ?"dir" : "file", obj->variant_type );

		kfree(obj);
	}
}

static int yaffs_remount(struct super_block *sb, int *flags, char *data) {
#ifdef CONFIG_HIBERNATION
	if (do_yaffs_remount) {
		struct yaffs_dev * dev;
		int init_result;

		//printk("yaffs_remount called  start !@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");

#if (LINUX_VERSION_CODE > KERNEL_VERSION(2, 5, 0))
		dev = sb->s_fs_info;
#else
		dev = sb->u.generic_sbp;
#endif

		if (!dev->is_mounted) {
			printk("yaffs_remount fail! isMounted %d\n", dev->is_mounted);
			return -EINVAL;
		}

		yaffs_gross_lock(dev);

		yaffs_inode_backup(sb);

		yaffs_deinitialise(dev);

		init_result = yaffs_guts_initialise(dev);

		yaffs_inode_restore(sb, dev);
		...
		//yaffs_print_directories(dev->root_dir);
		//printk("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");
		yaffs_set_incomplete_obj_all(dev);

		init_result = yaffs_guts_initialise(dev);
		//yaffs_print_directories(dev->root_dir);

		yaffs_reflash_inodes(sb, dev);

		//yaffs_print_directories(dev->root_dir);
		yaffs_check_incomplete_obj(dev);

		yaffs_print_directories(dev->lost_n_found);
		...
		*/
		...
		//printk("yaffs_remount called! end ##############################################################\n");

		if (init_result == YAFFS_OK)
			return 0;
