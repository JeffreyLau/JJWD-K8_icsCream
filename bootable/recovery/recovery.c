...
static const char *NAND_ROOT = "/nand";
static const char *USB_ROOT = "/usb";
static const char *SATA_ROOT = "/sata";
    ...
    for (;;) {
        ...
        switch (chosen_item) {
            ...
            case ITEM_APPLY_SDCARD:
                ...
                break;
	    case ITEM_APPLY_NAND:
                status = update_directory(NAND_ROOT, NAND_ROOT, &wipe_cache);
                if (status == INSTALL_SUCCESS && wipe_cache) {
                    ui_print("\n-- Wiping cache (at package request)...\n");
                    if (erase_volume("/cache")) {
                        ui_print("Cache wipe failed.\n");
            ...
            case ITEM_APPLY_CACHE:
                ...
                break;
            case ITEM_APPLY_USB:
                status = update_directory(USB_ROOT, USB_ROOT, &wipe_cache);
                if (status == INSTALL_SUCCESS && wipe_cache) {
                    ui_print("\n-- Wiping cache (at package request)...\n");
                    if (erase_volume("/cache")) {
                        ui_print("Cache wipe failed.\n");
                ...
                if (status >= 0) {
                    if (status != INSTALL_SUCCESS) {
                        ...
                        ui_print("\nInstall from usb complete.\n");
                    }
                }
                break;
             case ITEM_APPLY_SATA:
                status = update_directory(SATA_ROOT, SATA_ROOT, &wipe_cache);
                if (status == INSTALL_SUCCESS && wipe_cache) {
                    ui_print("\n-- Wiping cache (at package request)...\n");
                    if (erase_volume("/cache")) {
                        ui_print("Cache wipe failed.\n");
