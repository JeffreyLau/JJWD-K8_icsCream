...
/**************************************************************************
*  FUNCTION NAME : 
*      sHwGPSB *IO_GPSB_GetBaseAddr(unsigned uCH);
*  
*  DESCRIPTION : 
*  INPUT:
*			uCH	= Channel Number of GPSB Driver
*  
*  OUTPUT:	sHwGPSB - Return Type
*  			= 
*  REMARK  :	
**************************************************************************/
sHwGPSB *IO_GPSB_GetBaseAddr(unsigned uCH)
{
	return (sHwGPSB *)io_p2v(&HwGPSB_BASE_ADDR(uCH));
}
...
//	IO_CKC_SetGPSBClock(uCH, 900000);	//Dxb_TDMB_CHECK
	tca_ckc_setperi(PERI_GPSB0 + uCH ,ENABLE,  90000);
...
void IO_GPSB_DisableChannel(unsigned uCH)
{
	...
	{
		case DRV_GPSB_CH0:
			IO_CKC_DisableBUS_GPSB0();
			break;
		case DRV_GPSB_CH1:
			IO_CKC_DisableBUS_GPSB1();
			break;
		
 		case DRV_GPSB_CH2:
			IO_CKC_DisableBUS_GPSB2();
			break;
			
		case DRV_GPSB_CH3:
			IO_CKC_DisableBUS_GPSB3();
			break;
			
		case DRV_GPSB_CH4:
			IO_CKC_DisableBUS_GPSB4();
			break;
			
		case DRV_GPSB_CH5:
			IO_CKC_DisableBUS_GPSB5();
			break;
	}
}
						...
						unsigned *pucTxBuf, unsigned *pucRxBuf, unsigned uFIFOThr, unsigned uContMode)
{
	...
	pHwPIC	= (volatile PPIC)io_p2v(HwPIC_BASE);
	...
	if(pucTxBuf)
		pHwGPSB->TXBASE = pucTxBuf;					
	if(pucRxBuf)
		pHwGPSB->RXBASE = pucRxBuf;					
	...
	BITSET(pHwPIC->MODE1 , HwINT1_GPSB);   //use level triggered interrupt 
	BITSET(pHwPIC->IEN1, HwINT1_GPSB);    //Enable GPSB Interrupt		
