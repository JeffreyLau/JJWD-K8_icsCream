...
#elif defined(_TCC9300_) || defined(_TCC8800_) || defined(_TCC8800ST_) || defined(TCC892X)
...
void hdmi_phy_reset(void)
{
	...
	{
		phy_status = readb(HDMI_PHY_STATUS);
		DPRINTF("%s phy_status:%d regl:0x%08x\n", __func__, phy_status, regl);
...
int hdmi_set_phy_config(const struct HDMIVideoParameter * const hdmi_video_mode)
{
	...
	if(!phy_ready)	{
		DPRINTF("phy is not ready! reset :%d\n", phy_ready);
		hdmi_phy_reset();
 	}
#endif//
