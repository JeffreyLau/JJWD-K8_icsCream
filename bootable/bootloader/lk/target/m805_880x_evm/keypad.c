...
static unsigned int tcc9200s_col_gpios[] = {
	GPIO_PORTA|5, GPIO_PORTA|6, GPIO_PORTA|7
};

static unsigned int tcc9200s_row_gpios[] = {
	GPIO_PORTA|2, GPIO_PORTA|3, GPIO_PORTA|12
};

#define KEYMAP_INDEX(row, col) ((row)*ARRAY_SIZE(tcc9200s_col_gpios) + (col))
...
static const unsigned short tcc9200s_keymap[] = {
	...
	[KEYMAP_INDEX(2, 0)] = KEY_F1,
	[KEYMAP_INDEX(2, 1)] = KEY_UP,
	[KEYMAP_INDEX(2, 2)] = KEY_DOWN,
};

static struct gpio_keypad_info tcc9200s_keypad_info = {
...
void keypad_init(void)
{
	...
	gpio_keypad_init(&tcc9200s_keypad_info);
#endif
}
