...
#define TELECHIPS_MAC
extern char* atheros_wifi_mac; // telechips
                     ...
                     struct device *device)
{
	...
	*firmware_p = firmware = A_MALLOC(sizeof(*firmware));
    ...
    A_MEMZERO(firmware, sizeof(*firmware));
  	if (!strcmp(raw_filename, "softmac"))
		//sprintf(filename, "/data/%s", raw_filename);
		telechips_mac = 1;
	else
...
if(!telechips_mac)
{
	...
	if(atheros_wifi_mac){
		memcpy(firmware->data, atheros_wifi_mac, 17);
		//firmware->data[17] = '\0';
		length = 17;
		//printk("########## %s\n", firmware->data);
	}
