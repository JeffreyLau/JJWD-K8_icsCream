...
struct {
    ...
    { "log.",             AID_SHELL,    0 },
    { "tcc.",             AID_SYSTEM,   0 },
    { "tcc.video.",       AID_MEDIA,    0 },
    { "tcc.audio.",       AID_MEDIA,    0 },	// SPDIF Planet 20120203
    { "tcc.camera.",       AID_MEDIA,    0 },
    { "persist.sys.",     AID_MEDIA,   0 },
...
static int check_perms(const char *name, unsigned int uid, unsigned int gid)
{
    ...
    if(!strncmp(name, "wifi.", 5))
        return 1;

    if(!strncmp(name, "tcc.", 4))
        return 1;

    if(!strncmp(name, "tcc.all.", 8))
	return 1;
