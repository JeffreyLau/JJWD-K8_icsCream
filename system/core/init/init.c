...
static void import_kernel_nv(char *name, int in_qemu)
{
    ...
    *value++ = 0;
    ...
    if (!in_qemu)
    {
        ...
        } else if (!strcmp(name, "androidboot.btaddr")) {
            strlcpy(btaddr, value, sizeof(btaddr));
        } else if (!strcmp(name,"androidboot.modelno")) {
            strlcpy(modelno, value, sizeof(modelno));
        } else if (!strcmp(name, "androidboot.bootmode")) {
					strlcpy(bootmode, value, sizeof(bootmode));
				}
    } else {
...
int main(int argc, char **argv)
{
    ...
    klog_init();
			...
			//init_parse_config_file("/init.factory.rc");
    import_kernel_cmdline(0, import_kernel_nv);
		
		if( !strcmp(bootmode, "tcc_factory")) {
			NOTICE("reading tcc factory test mode config file\n");
			init_parse_config_file("/init.factory.rc");
		} else {
    ...
    /* XXX: execute some actions before early init */
    action_for_each_trigger("init-start", action_add_queue_tail);
