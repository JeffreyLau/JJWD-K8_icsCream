...
void Volume::setState(int state) {
    ...
    SLOGW("setState: oldstate(%d) -> newstate(%d)", oldState, state);
...
int Volume::createDeviceNode(const char *path, int major, int minor) {
    //SLOGI("==========================================");	
    //SLOGI("createDeviceNode::(%s) Major(%d) Minor(%d)", path, major, minor);
    //SLOGI("==========================================");
...
bool Volume::isMountpointMounted(const char *path) {
    ...
    //SLOGI("==========================================");	
    //SLOGI("isMountpointMounted::(%s)", path);
    //SLOGI("==========================================");
...
int Volume::mountVol() {
    ...
    int n, i, rc = 0;
    char errmsg[255];
    //B090162 : export vairable in init.tcc8920.rc
    ...
    n = getDeviceNodes((dev_t *) &deviceNodes, MAX_MOUNT_PART);
    if (!n) {
    ...
    for (i = 0; i < n; i++) {
        ...
        if (primaryStorage) {
        	...
        	//B090162: if primary storage is ext_sd, permission problem in gallery3d 
        	if (!strcmp(externalStorage, "/mnt/ext_sd"))
        		gid = AID_SDCARD_RW;
        	else	
        ...
        if (mountPartition(devicePath, "/mnt/secure/staging", gid) != 0) {
            continue;
        }
        ...
        if (mMultiMount) {
            ...
            while (++i < n) {
                ...
                if (mountPartition(devicePath, mountPoint, gid) == 0) {
                    mounted++;
                }
            }
        }
...
int Volume::mountPartition(char *devicePath, char *mountPoint, int gid) {
...
int Volume::doMoveMount(const char *src, const char *dst, bool force) {
    ...
    //SLOGI("==========================================");	
    //SLOGI("doMoveMount::src(%s) - dst(%s)", src, dst);
    //SLOGI("==========================================");
