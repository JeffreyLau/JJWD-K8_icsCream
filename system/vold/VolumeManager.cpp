...
#define MASS_STORAGE_SDCARD_FILE_PATH  "/sys/class/android_usb/android0/f_mass_storage/lun0/file"
#define MASS_STORAGE_EXT_SD_FILE_PATH  "/sys/class/android_usb/android0/f_mass_storage/lun1/file"
...
void VolumeManager::handleBlockEvent(NetlinkEvent *evt) {
    ...
    SLOGD("==========================================");
    SLOGD("VolumeManager::handleBlockEvent");	
    SLOGD("==========================================");
...
int VolumeManager::listVolumes(SocketClient *cli) {
	...
	SLOGD("==========================================");
	SLOGD("VolumeManager::listVolumes");	
	SLOGD("==========================================");
...
int VolumeManager::getAsecMountPath(const char *id, char *buffer, int maxlen) {
    ...
    //SLOGI("==========================================");	
    //SLOGI("getAsecMountPath::asecFileName(%s)", asecFileName);
    //SLOGI("==========================================");
	...
	SLOGW("getAsecMountPath: %s", buffer);	
    return 0;
}
...
int VolumeManager::createAsec(const char *id, unsigned int numSectors,
    ...
    snprintf(asecFileName, sizeof(asecFileName), "%s/%s.asec", Volume::SEC_ASECDIR, id);
    //SLOGI("==========================================");	
    //SLOGI("createAsec::(%s)", asecFileName);
    //SLOGI("==========================================");
    if (!access(asecFileName, F_OK)) {
...
int VolumeManager::unmountAsec(const char *id, bool force) {
    ...
    snprintf(mountPoint, sizeof(mountPoint), "%s/%s", Volume::ASECDIR, id);
    //SLOGI("==========================================");	
    //SLOGI("unmountAsec::asecFileName(%s), mountPoint(%s)", asecFileName, mountPoint);
    //SLOGI("==========================================");
    char idHash[33];
    if (!asecHash(id, idHash, sizeof(idHash))) {
        SLOGE("Hash of '%s' failed (%s)", id, strerror(errno));
...
int VolumeManager::destroyAsec(const char *id, bool force) {
    ...
    snprintf(mountPoint, sizeof(mountPoint), "%s/%s", Volume::ASECDIR, id);
    //SLOGI("==========================================");	
    //SLOGI("destroyAsec::asecFileName(%s), mountPoint(%s)", asecFileName, mountPoint);
    //SLOGI("==========================================");
    if (isMountpointMounted(mountPoint)) {
        if (mDebug) {
...
int VolumeManager::mountAsec(const char *id, const char *key, int ownerUid) {
    ...
    snprintf(mountPoint, sizeof(mountPoint), "%s/%s", Volume::ASECDIR, id);
    //SLOGI("==========================================");	
    //SLOGI("mountAsec::asecFileName(%s), mountPoint(%s)", asecFileName, mountPoint);
    //SLOGI("==========================================");
    if (isMountpointMounted(mountPoint)) {
...
int VolumeManager::mountVolume(const char *label) {
    Volume *v = lookupVolume(label);
    SLOGI("==========================================");	
    SLOGI("mountVolume::Label(%s)", label);
    SLOGI("==========================================");  
    if (!v) {
        errno = ENOENT;
        return -1;
...
int VolumeManager::shareEnabled(const char *label, const char *method, bool *enabled) {
    ...
    Volume *v = lookupVolume(label);
    SLOGI("==========================================");	
    SLOGI("shareEnabled::Label(%s)", label);
    SLOGI("==========================================");  
    if (!v) {
        errno = ENOENT;
        return -1;
...
int VolumeManager::shareVolume(const char *label, const char *method) {
    ...
    Volume *v = lookupVolume(label);
    //SLOGI("==========================================");	
    //SLOGI("shareVolume::Label(%s)", label);
    //SLOGI("==========================================");  
    if (!v) {
        errno = ENOENT;
        return -1;
    ...
    if (!strcmp(v->getMountpoint(), "/mnt/sdcard")) {
       if ((fd = open(MASS_STORAGE_SDCARD_FILE_PATH, O_WRONLY)) < 0) {
          SLOGE("Unable to open ums lunfile (%s)", strerror(errno));
          return -1;
       }
    } else if(!strcmp(v->getMountpoint(), "/mnt/ext_sd")) { 
       if ((fd = open(MASS_STORAGE_EXT_SD_FILE_PATH, O_WRONLY)) < 0) {
          SLOGE("Unable to open ums lunfile (%s)", strerror(errno));
          return -1;
       }
    } else {
...
int VolumeManager::unshareVolume(const char *label, const char *method) {
    ...
    Volume *v = lookupVolume(label);
    //SLOGI("==========================================");	
    //SLOGI("unshareVolume::Label(%s)", label);
    //SLOGI("==========================================");  
    if (!v) {
        errno = ENOENT;
        return -1;
...
int VolumeManager::cleanupAsec(Volume *v, bool force) {
    ...
    const char* externalStorage = getenv("EXTERNAL_PRIMARY_STORAGE");
