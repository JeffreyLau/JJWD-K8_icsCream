...
//#define SUPPORT_LOGICAL_PARTITION
...
#endif
		if(!strcmp(evt->findParam("DEVNAME"),"ndda"))
			return;
        setState(Volume::State_Idle);
    } else {
#ifdef PARTITION_DEBUG
...
SLOGW("====>> %s", devpath);
...
#ifdef SUPPORT_LOGICAL_PARTITION
    if (!mPendingPartMap) {
        SLOGE("Too much the event for adding a partition(part num:%d)", part_num);
        return;
    }
#else
    ...
    if (part_num >= MAX_PARTITIONS) {
        ...
        mPartMinors[mDiskNumParts - mPendingPartMap] = minor;
    }
    mPendingPartMap--;
#else
    } else {
...
#endif
        if (getState() != Volume::State_Formatting && getState() != Volume::State_Mounted) {
            setState(Volume::State_Idle);
            if (mRetryMount == true) {
                mRetryMount = false;
                mountVol();
 ...
 */
void DirectVolume::UnmountUnshare(int major, int minor) {
    char msg[255];
    int state;
    //SLOGI("==========================================");	
    //SLOGI("UnmountUnshare::Major(%d) Minor(%d)",  major, minor);
    //SLOGI("==========================================");
    state = getState();
	...
	if (mVm->cleanupAsec(this, true)) {
        ...
        //B090162: if ext_sd is primary storage?	
        ...
        //B090162:empty getMountpoint argument
        snprintf(msg, sizeof(msg), "Volume %s %s bad removal (%d:%d)",
                 getLabel(), getMountpoint(), major, minor);
        mVm->getBroadcaster()->sendBroadcast(ResponseCode::VolumeBadRemoval,
                                             msg, false);
...
int DirectVolume::updateDeviceInfo(char *new_path, int new_major, int new_minor)
{
    PathCollection::iterator it;
    //SLOGI("==========================================");	
    //SLOGI("updateDeviceInfo::Path(%s) Major(%d) Minor(%d)", new_path,  new_major, new_minor);
    //SLOGI("==========================================");
    if (mPartIdx == -1) {
...
{
    //SLOGI("==========================================");	
    //SLOGI("setStorageType::Type(%d) MountPoint(%s) Main ExtStorage(%s)", type, getMountpoint(), getenv("EXTERNAL_STORAGE"));
    //SLOGI("==========================================");
    
    if (strcmp(getMountpoint(), getenv("EXTERNAL_PRIMARY_STORAGE"))) {
        return;
    }
